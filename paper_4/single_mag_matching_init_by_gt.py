...
def main():
    ...
    for i in range(0, len(match_seq_list)):
        ...
        # TODO 在这里，根据PDRindex找到GT段对应的xy，计算PDR段与GT段之间的transf0=[x, y, angle]（使用PDR_IMU_ALIGN_SIZE进行对齐）
        # ①先获取pdr_tray与gt_tary
        pdr_tray = match_seq[:, 0:2]  # [N][x,y]
        gt_tray = []
        for d in match_seq:
            gt_tray.append(np.array(gt_xy[d[4]*PDR_IMU_ALIGN_SIZE]))
        # TODO ②根据adjust_pdr_by_markpoints.py计算初始transf0
