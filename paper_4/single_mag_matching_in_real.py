...
# PATH_PDR_RAW = ['../data/XingHu hall 8F test/position_test/8/IMU-88-8-189.88230883318997 Pixel 6_sync.csv.npy',
...
def main():
    ...
    # TODO 从文件中读取区域特征数组
    # feature_fc_map = MMT.rebuild_map_from_mvh_files(["paper_4/results/mv_map_features/InfCenter/fc.csv",
    #                                                  "paper_4/results/mh_map_features/InfCenter/fc.csv"])
    # feature_td_map = MMT.rebuild_map_from_mvh_files(["paper_4/results/mv_map_features/InfCenter/td.csv",
    #                                                  "paper_4/results/mh_map_features/InfCenter/td.csv"])
    # feature_xgx_map = MMT.rebuild_map_from_mvh_files(["paper_4/results/mv_map_features/InfCenter/xgx.csv",
    #                                                   "paper_4/results/mh_map_features/InfCenter/xgx.csv"])
    ...
    for i in range(0, len(match_seq_list)):
        ...
        #     # 根据当前定位窗口结果xy，从指纹库中获取mv、mh，grad_v、grad_h
        ...
        # mag_vh_arr = match_seq[:, 2:4]
        # seq_slen = 1  # 序列特征尺度（米）
        # seq_v_fc, seq_v_td, seq_v_xgx = MF.get_seq_feature_seqs(mag_vh_arr[:, 0], seq_slen/DOWN_SIP_DIS)
        # seq_h_fc, seq_h_td, seq_h_xgx = MF.get_seq_feature_seqs(mag_vh_arr[:, 1], seq_slen/DOWN_SIP_DIS)
        #
        # # TODO 得到的特征和xy应该是一一对应的，
        # #  for循环计算单点xy误差，
        # #  与对应的单点特征一起保存至结果文件
        # print(len(distance_of_MagPDR_iLocator_points), ",", len(seq_v_fc))
